...
namespace GraphPlayground
{
    internal class Graph
    {
        ...
        public Graph()
        {
            ...
            nodes = new List<Node>();
            for (int i = 0; i < rng.Next(5, 10); i++) //vytvoří se random počet uzlů od 5 do 10
            {
                nodes.Add(new Node(i));
            }

            //Randomly add neighbors to nodes with some limitations of the amount of neighbors for each node
            foreach (Node node in nodes) //rychlé přidání náhodných hran
            {
        ...
        public void PrintGraph() //použe to printuje, vypíše to uzel s jeho indexem a jeho sousedy
        {
        ...
        public void PrintGraphForVisualization() //vytiskne všechyn hrany v grafu 
        {
            foreach (Node node in nodes)
            {
                foreach (Node neighbor in node.neighbors)
